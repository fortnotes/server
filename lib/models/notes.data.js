 ...
 */
notesData.add = function ( token, noteData, callback ) {
			...
			db.models.sessions.check(token, function ( error, session ) {
				...
				db.models.notes.get(noteData.id, function ( error, note ) {
					...
					note.save({updateTime: time}, function ( error, note ) {
						...
						// add note data
						notesData.create({noteId: note.id, data: noteData.data, hash: noteData.hash, createTime: time}, function ( error, note ) {
							if ( error ) {
								console.log(error);
