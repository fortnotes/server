...
notes.list = function ( token, params, callback ) {
	...
	db.models.sessions.check(token, function ( error, session ) {
		...
		// data window
		params.limit  = Math.abs(Number(params.limit))  || global.config.dataLimit;
		params.limit  = Math.min(params.limit, global.config.dataLimitMax);
...
notes.insert = function ( token, params, callback ) {
	...
	if ( params.data && params.hash ) {
		...
		if ( params.data.length <= global.config.dataSize && params.hash.length <= global.config.hashSize ) {
			// is user authorized
			db.models.sessions.check(token, function ( error, session ) {
				var time = +new Date();
