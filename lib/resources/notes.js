...
/**
 * @api {get} /notes?limit=:limit&offset=:offset Receive a list of user notes.
 *
 * @apiVersion 1.0.0
 * @apiName getNotes
 * @apiGroup Notes
...
restify.get('/notes',
	function ( request, response ) {
		notes.list(request.authorization.token, request.params, function ( error, data ) {
			if ( error ) {
...
/**
 * @api {get} /notes/:id?limit=:limit&offset=:offset Get a note old values list.
 *
 * @apiVersion 1.0.0
 * @apiName getNote
 * @apiGroup Note
...
restify.get('/notes/:id',
	function ( request, response ) {
		notesHistory.list(request.authorization.token, request.params, function ( error, data ) {
			if ( error ) {
				return response.send(error);
			}
