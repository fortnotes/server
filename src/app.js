        ...
        self.db.sync(function ( error ) {
            var corsMiddleware = require('restify-cors-middleware'),
                cors = corsMiddleware({
                    credentials: true,
                    // preflightMaxAge: 5,
                    //origins: ['http://10.0.0.100:8080/'],
                    // allowHeaders: ['API-Token'],
                    // exposeHeaders: ['API-Token-Expiry']
                    preflightMaxAge: 25,
                    origins: ['http://10.0.0.100:8080/', 'http://127.0.0.1:8080/', 'http://localhost:8080/'],
                    allowHeaders: ['*'],
                    exposeHeaders: ['*']
                });
            ...
            self.restify.get('/user/info', function ( request, response ) {
                response.send(null);
            });

            self.restify.post('/user', function ( request, response ) {
                console.log(request.body);
                response.send(request.body);
            });

            // plugins
            self.restify.use(restify.plugins.acceptParser(self.restify.acceptable));
            self.restify.use(restify.plugins.bodyParser());
            self.restify.use(restify.plugins.queryParser());
            ...
            self.restify.pre(cors.preflight);
            self.restify.use(cors.actual);
        });
    });
}
